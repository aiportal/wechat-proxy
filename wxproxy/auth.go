...
func (srv *WechatAuthServer) getBaseInfo(p *authRequestParam, code string) (info *WechatUserInfo, err error) {
	var t authTokenInfo
	tokenUrl := srv.wechatTokenUrl(p, code)
	_, err = httpGetJson(tokenUrl, &t)
	if err != nil {
		return
	}
	if !t.Success() {
	...
	info, err = c.getUserInfo(t.OpenId, p.Lang)
	return
}

func (srv *WechatAuthServer) getUserInfo(p *authRequestParam, code string) (info *WechatUserInfo, err error) {
	var t authTokenInfo
